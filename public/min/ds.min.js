function loadFile(e,t){if(ft.ongoingXhrId=e,!$("#doc-"+ft.selected.id).length||$("#doc-"+ft.selected.id).attr("id").substring(4)!=e||ft.selected.id!=e){$("#doc-"+ft.selected.id).hide()
var n=$("#doc-"+e)
...
n+=ft.selected.text}n+="</article>",$(".doc").hide(),docs.insertAdjacentHTML("beforeend",n)}delete ft.selected.text,$("#doc-"+ft.selected.id).data("info",ft.selected),$("#"+ft.selected.id).hasClass("collapsed")&&$("#"+ft.selected.id).parent().find(".ft-icon").click(),$("#"+ft.selected.id).length||(u.index||(u.index=[]),u.index.push({id:ft.selected.id,title:ft.selected.title,type:"ft-file ft-selected"}),ft.populate($("#filetree"),u.index)),t&&t(),$("#select2-searchtext-container").html(ft.selected.title),ft.updateNeeded=!0,setTimeout(ft.update,2e3)}})}}function pasteHtmlAtCaret(e){sel=getSelection(),range=sel.getRangeAt(0),range.deleteContents(),el=range.createContextualFragment(e)
for(var t,n,i=document.createDocumentFragment();t=el.firstChild;)n=i.appendChild(t)
var o=i.firstChild
range.insertNode(i),n&&(range=range.cloneRange(),range.setStartAfter(n),range.setStartBefore(o),sel.removeAllRanges(),sel.addRange(range))}function loadFile(e,t){if(ft.ongoingXhrId=e,!$("#doc-"+ft.selected.id).length||$("#doc-"+ft.selected.id).attr("id").substring(4)!=e||ft.selected.id!=e){$("#doc-"+ft.selected.id).hide()
var n=$("#doc-"+e)
if(n.length)return n.show(),ft.selected=n.data("info"),$(".opened").removeClass("opened"),$("#"+e).addClass("opened"),$("#"+e).hasClass("collapsed")&&$("#"+e).parent().find(".ft-icon").click(),ft.selected&&(history.pushState(null,"",ft.selected.id),$("#select2-searchtext-container").html(ft.selected.title)),void(t&&t())
$(".glyphicon-refresh").removeClass("glyphicon-refresh glyphicon-spin"),$("#"+e+" ").prev().find(".ft-icon").addClass("glyphicon-refresh glyphicon-spin"),$(".opened").removeClass("opened"),ft.selected={},$.ajax({xhr:function(){var t=new window.XMLHttpRequest
return t.addEventListener("progress",function(n){if(n.lengthComputable){var i=Math.floor(n.loaded/n.total*100)
...
for(var t,n,i=document.createDocumentFragment();t=el.firstChild;)n=i.appendChild(t)
var o=i.firstChild
range.insertNode(i),n&&(range=range.cloneRange(),range.setStartAfter(n),range.setStartBefore(o),sel.removeAllRanges(),sel.addRange(range))}function round_init(){$.getJSON("/round",function(e){$("#pastrounds").empty(),e.length&&$("#pastrounds").addClass("well")
for(var t in e){var n=$("<div>")
n.attr("id",e[t]._id),n.html("<a class='label label-default'>"+new Date(e[t].date_created).toLocaleDateString()+"</a> "+e[t].aff1.name+" "+e[t].aff2.name+" <strong>vs</strong> "+e[t].neg1.name+" "+e[t].neg2.name+" <strong>judged by</strong> "+e[t].judges.map(function(e){return e.name}).join(", ")),n.click(function(){r._id=$(this).attr("id"),startRound()}),$("#pastrounds").append(n)}})}function startRound(){$("#round").is(":visible")||$("#showround").click(),$.getJSON("/round/read",{id:r._id},function(e){r=e,$("#speech1AC").html(r.speech1AC.text),$("#speech1NC").html(r.speech1NC.text),$("#speech2AC").html(r.speech2AC.text),$("#speech2NC").html(r.speech2NC.text),$("#speech1NR").html(r.speech1NR.text),$("#speech1AR").html(r.speech1AR.text),$("#speech2NR").html(r.speech2NR.text),$("#speech2AR").html(r.speech2AR.text),$(".username-select").empty(),$("#aff1").append("<option value='"+r.aff1.id+"'selected>"+r.aff1.name+"</option>"),$("#aff2").append("<option value='"+r.aff2.id+"'selected>"+r.aff2.name+"</option>"),$("#neg1").append("<option value='"+r.neg1.id+"'selected>"+r.neg1.name+"</option>"),$("#neg2").append("<option value='"+r.neg2.id+"'selected>"+r.neg2.name+"</option>")
...
for(var e=$(".tab-content .active p"),t=0;t<e.length&&!(e[t].getBoundingClientRect().top>0);t++);0==u.big?($(".doc:visible, .speech").find("*[style]").filter(function(){return"rgba(0, 0, 0, 0)"!=$(this).css("background-color")}).css("font-size","").css("line-height",""),$("#docs, .speech").find("*[style*='yellow'], b, u, h1").css("font-size","").css("line-height",""),$(".doc:visible, .speech").css("font-size","100%")):1==u.big?($(".doc:visible, .speech").find("*[style]").filter(function(){return"rgba(0, 0, 0, 0)"!=$(this).css("background-color")}).css("font-size","30pt").css("line-height","100%"),$(".doc:visible, .speech").find("*[style*='yellow'], b, u, h1").css("font-size","30pt").css("line-height","100%"),$(".doc:visible, .speech").css("font-size","30%")):2==u.big&&$(".doc:visible, .speech").find("u").css("font-size","").css("line-height","")}),$("#showround").click(function(){$("#round").toggle(),$("#docs").css("padding-right",$("#round").is(":visible")?"45%":0),$("#round").is(":visible")&&round_init()}),$("#file-new").click(function(){$("#modal-file-new").show(),$("#file-new-modal").on("shown.bs.modal",function(){$("#filename").focus().keydown(function(e){13==e.keyCode&&$("#file-new-modal-submit").click()})})}),$("#file-new-modal-submit").click(function(){if("file"==$(".filetype:checked").val()){var e={type:"ft-file ft-selected",title:$("#filename").val(),text:""}
$.get("/doc/create",e,function(t){e.id=t._id,u.index.push(e),ft.populate(ft.root,u.index)}),$(".ft-name:last").click()}else{var t={type:"folder",userid:"local",title:$("#filename").val()}
u.index.push(t),ft.populate(ft.root,u.index),$(".ft-name:last").click()}$("#modal-file-new").hide()}),$("#import_googledrive").click(function(){$("#modal-file-new").hide(),window.google?e():$("head").append('<script src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.0.0/jszip.min.js" ></script><script src="https://apis.google.com/js/client.js?onload=gapiInit" ></script>')}),window.gapiInit=function(){gapi.key="AIzaSyDgdM5CpdzE3dLHD877L8fB3PyxVpV7pY4",gapi.client.setApiKey(gapi.key),gapi.auth.authorize({client_id:"675454780693-7n34ikba11h972dgfc0kgib0id9gudo8.apps.googleusercontent.com",scope:["profile","email","https://www.googleapis.com/auth/drive"],immediate:!0},function(t){console.log(t),gapi.authToken=t.access_token,gapi.client.load("drive","v2",function(){}),gapi.load("picker",{callback:e})})}}),window.chunk=!0,$(window)[0].addEventListener("paste",function(e){},!0),$(document).ready(function(){function e(e){u.index&&u.index.filter(function(t){return t.id==e}).length&&ft.selected.id!=e?$("#"+e).click():loadFile(decodeURIComponent(e).replace(/[\W]+/g,""))}if(u){var t=$("<style>").appendTo("head")
t.html(u.custom_css)
...
for(var i in u.notifications)"round_youAreInvited"==u.notifications[i].type?roundInviteAlert(u.notifications[i]):"doc_share"==u.notifications[i].type&&docShareAlert(u.notifications[i])}else $("#login").show().click(function(){location.href="/login"}),location.pathname.length<2?($("#showround").click(),$("#ft-minimize-unread").click(),u={index:JSON.parse('[{"id":"manual","title":"Debate Synergy Manual","type":"file selected","children":[{"title":"Welcome to Debate Synergy","type":"h h1"},{"title":"Manual","type":"h h1"},{"title":"Debate Sidebar Word AddIn ","type":"h h1"}]}]')}):u={index:[]}
ft.init($("#filetree"),u.index),setTimeout(function(){$.get("/index_res.html",function(e){$("body").append(e)})},2e3),$("#ft-minimize-unread").click(),$(window).on("popstate",loadFile)
var o=location.pathname.substr(1)
o&&"#"!=o?e(o):u.name&&"manual"!=u.index[0].id?$(".selected,.opened").length&&$(".selected,.opened").eq(0).click():($(".ft-item:first").click(),$("#docs").load("/res/manual.html"),$.get("/doc/read",{id:"manual"},function(e){$("#docs").html(e)}),window.scrollTo(0,0)),$("body").on("touchstart",function(e){window.touchStart=e.originalEvent.changedTouches[0].pageX}).on("touchend",function(e){var t=window.touchStart?e.originalEvent.changedTouches[0].pageX-window.touchStart:0;-100>t?($("#sidebar").animate({marginLeft:"-"+$("#sidebar").width()},250),$("#docs").animate({paddingLeft:"0"},250)):t>100&&($("#sidebar").animate({marginLeft:"0"},250),$("#docs").animate({paddingLeft:"20%"},250))}),$(document).width()<700,window.dragSidebar=!1,$("#sidebar").on("mousemove",function(e){$(this).width()-e.offsetX<10?$("body").css("cursor","e-resize"):dragSidebar||$("body").css("cursor","")}).on("mouseout",function(e){dragSidebar||$("body").css("cursor","")}).on("mousedown touchstart",function(e){var t=e.originalEvent.touches?e.originalEvent.touches[0].clientX:e.offsetX
...
return oldX=xPos,oldY=yPos,xPos=t.pageX,yPos=t.pageY,null==oldX&&null==oldY?!1:Math.abs(oldX-xPos)>Math.abs(oldY-yPos)?(e.preventDefault(),!1):void 0}),$(window).keydown(function(e){83==e.which&&e.ctrlKey&&(e.preventDefault(),ft.update()),e.keyCode>=112&&e.keyCode<120?(e.preventDefault(),$("#controls button")[e.keyCode-112].click()):27==e.keyCode&&$("#searchtext").select2("open")}),$(window).on("beforeunload",function(){}),NodeList.prototype.forEach=Array.prototype.forEach
var ft={init:function(e,t){function n(){var e=document.elementFromPoint(filetree.offsetWidth+20,filetree.getBoundingClientRect().top+.2*filetree.clientHeight+30),t=$(e).closest("section>*").eq(0).prevUntil("h1,h2,h3").last(),n=$("#doc-"+ft.selected.id).find("h1, h2, h3")
if(this.worked=0,t){for(var i=0;i<n.length;i++)if(n.eq(i).text().length>0&&n.eq(i).is(t)){this.worked=1
break}this.worked&&($(".selected").removeClass("selected"),$("#"+ft.selected.id).find("li").eq(i-2).addClass("selected"),$(".selected")[0]&&document.body.scrollIntoViewIfNeeded&&$(".selected")[0].scrollIntoViewIfNeeded())}}ft.root=$(e),ft.dragging=!1,ft.selected={},ft.populate(e,t),$("#filetree").on("click","ul,li",ft.click),$("#filetree").on("click","label",function(e){var t=$(this).parent()
t.toggleClass("collapsed")}),$("#filetree").scroll(function(){this.scrollLeft=0}),$(window).on("scroll",function(){clearTimeout(window.scrollThrottle),window.scrollThrottle=setTimeout(n,500)}),setInterval(function(){ft.update()},2e4),ft.updateNeeded=!1,$("#docs").keyup(function(e){ft.updateNeeded=!0,$(window.getSelection().anchorNode).closest("h1,h2,h3").length&&(ft.updateNeeded=!0)}),$("#filetree").on("dragstart",function(e){ft.dragging=$(e.target).closest("nav"),$(e.target).addClass("ft-being-dragged")}),$(document).on("dragover",function(e){e.preventDefault()}),$("#filetree").on("dragenter",function(e){e=$(e.target).closest("nav"),e&&e.addClass("ft-dragged-over")}).on("dragleave",function(e){e=$(e.target).closest("nav"),e&&e.removeClass("ft-dragged-over")}).on("drop",function(e){if(!e.originalEvent.dataTransfer||!e.originalEvent.dataTransferuls.length){e.preventDefault()
...
var o=parseInt(t.find(".ft-name").attr("id").substring(t.find(".ft-name").attr("id").indexOf("_")+1)),a=$("#doc-"+ft.selected.id).find("h1,h2,h3"),s=a.eq(o+1),l=a.eq(i)[0],c=a.eq(1+i).prev()[0],d=document.createRange()
d.selectNodeContents(l),d.setEnd(c,c.childNodes.length)
var r=$("<span>")
r.append(d.extractContents()),console.log(r.html()),s.before(r)}}else $("#filetree > .ft-list > .ft-item:last").after(ft.dragging)
$(".ft-being-dragged:first").closest(".ft-item").remove(),u.index=ft.toJSON(),ft.updateIndex()}})},populate:function(e,t){function n(e){var t=""
for(var i in e){var o=e[i],a=""
o.type=(o.type||"").replace(/heading /g,"h ").replace(/heading-/g,"").replace(/ft-/g,""),!o.children&&o.type.indexOf("collapsed")>-1&&(o.type+=" collapsed"),o.type.indexOf("file")>-1&&(a="<ul draggable class='"+o.type+"' id='"+o.id+"'><label>"+o.title+"</label>"),o.type.search(/h\d/i)>-1&&(a="<li draggable class='"+o.type+"'><label>"+o.title+"</label>"),o.children&&o.children.length&&(a+=n(o.children)),o.type.indexOf("file")>-1&&(a+="</ul>"),o.type.search(/h\d/i)>-1&&(a+="</li>"),t+=a}return t}ft.root&&e[0]&&e[0].id==ft.root[0].id,outputHTML=n(t),e.append(outputHTML)},toJSON:function(e){"undefined"==typeof e&&(e=filetree)
var t=[]
return $(e).children("li").each(function(){var e=$(this),n={id:e.attr("id"),title:e.children("label").text(),type:e.attr("class")},i=e.find("li")
i.length?n.children=ft.toJSON(i):n.type+=" collapsed",t.push(n)}),t},update:function(){if(u.index=ft.toJSON(),ft.selected&&$("#doc-"+ft.selected.id).length&&ft.updateNeeded&&ft.selected.id==$("#doc-"+ft.selected.id).attr("id").substring(4)){var e=[],t=0,n=-1
...
break}console.log(e),$("#"+ft.selected.id).find("li").remove(),ft.populate($("#"+ft.selected.id),e),$.post({url:"/doc/update",contentType:"application/json",data:JSON.stringify({text:$("#doc-"+ft.selected.id).html(),id:ft.selected.id})})}ft.updateIndex()},updateIndex:function(){u.index.length&&u.name&&$.post({url:"/user/update",data:JSON.stringify({userid:u._id,index:u.index}),contentType:"application/json"})},click:function(e){function t(){$("#doc-"+ft.selected.id).find("h1, h2, h3")[a].scrollIntoView(),$("#doc-"+ft.selected.id).scrollTop($("#doc-"+ft.selected.id).scrollTop()-260)}e.stopPropagation()
...
ft.selected.id==o?t():loadFile(o,t)}else n.hasClass("file")&&loadFile(n.attr("id"))}}
$(document).ready(function(){function e(e){u.index&&u.index.filter(function(t){return t.id==e}).length&&ft.selected.id!=e?$("#"+e).click():loadFile(decodeURIComponent(e).replace(/[\W]+/g,""))}if(u){var t=$("<style>").appendTo("head")
t.html(u.custom_css)
var n=$("<script id='custom_script'>").html(u.custom_js)
n.appendTo("head"),u.options&&u.options.sidebar&&$("#sidebar").css("max-width",u.options.sidebar+"px"),u.options&&3==u.options.debatetype&&($(".nav-item:has(.nav-link[href='#speech2AC']), .nav-item:has(.nav-link[href='#speech2NC']), .nav-item:has(.nav-link[href='#speech1NR'])").remove(),$(".nav-link[href='#speech1AC']").text("AC"),$(".nav-link[href='#speech1NC']").text("NC"),$(".nav-link[href='#speech2NR']").text("NR"),$("#aff2+span,#neg2+span").hide())
for(var i in u.notifications)"round_youAreInvited"==u.notifications[i].type?roundInviteAlert(u.notifications[i]):"doc_share"==u.notifications[i].type&&docShareAlert(u.notifications[i])}else $("#login").show().click(function(){location.href="/login"}),location.pathname.length<2?($("#showround").click(),$("#ft-minimize-unread").click(),u={index:JSON.parse('[{"id":"manual","title":"Debate Synergy Manual","type":"file selected","children":[{"title":"Welcome to Debate Synergy","type":"h h1"},{"title":"Manual","type":"h h1"},{"title":"Debate Sidebar Word AddIn ","type":"h h1"}]}]')}):u={index:[]}
...
var xPos=null,yPos=null
window.addEventListener("touchmove",function(e){var t=e.touches[0]
return oldX=xPos,oldY=yPos,xPos=t.pageX,yPos=t.pageY,null==oldX&&null==oldY?!1:Math.abs(oldX-xPos)>Math.abs(oldY-yPos)?(e.preventDefault(),!1):void 0}),$(window).keydown(function(e){83==e.which&&e.ctrlKey&&(e.preventDefault(),ft.update()),e.keyCode>=112&&e.keyCode<120?(e.preventDefault(),$("#controls button")[e.keyCode-112].click()):27==e.keyCode&&$("#searchtext").select2("open")}),$(window).on("beforeunload",function(){}),window.chrome&&window.chrome.runtime,1
var r={}
initSockets(),$(document).ready(function(){$("#judges, #aff1, #aff2, #neg1, #neg2").select2({ajax:{url:"/user/search",dataType:"json",delay:50,data:function(e){return{userinfo:e.term}},processResults:function(e,t){return{results:e.splice(0,u.name?5:0)}},cache:!0},minimumInputLength:2,escapeMarkup:function(e){return e},maximumSelectionLength:9,templateResult:function(e){return e.email?u.name?"<span><b>"+e.text+"</b> "+e.email+"</span>":"Login required":e.text}}),$("#round").on("mousedown click",".flow-mode",function(e){console.log(e)
...
else{var n=$("<div class='flow'><textarea class='flow-text'></textarea> <div class='fa fa-2x fa-chevron-circle-down flow-toggle'></div>   <div class='flow-link fa fa-2x fa-link'  data-placement='left' title='Click Flow Anchor, then select a response in another speech to scroll to, when you hover over this flow entry'></div></div>")
...
e.data("flow-anchor",t),$(this).addClass("flow-anchor-"+t)}))}),$("#round").on("scroll",".speech",function(){}),$("#round").on("mouseover mousemove",".flow",function(){var e=$(this).find(".flow-link")
...
e.length||$(this).parent().remove()}),$(".speech").on("scroll",function(){if("false"!=$(this).attr("contenteditable")&&$("li.active a").hasClass("btn btn-info")){console.log($(this).attr("contenteditable"))
...
console.log(o),e>o&&(o=e),$(this).attr("scroll",o),$.get("round/updateScroll",{roundId:r._id,speechName:$(this).attr("id"),scrollTo:o})}}),$(".speech-nav").click(function(e){var t=$(this).attr("id").replace("-nav","")
...
for(var t in e)$("#"+e[t]).addClass("btn-danger")}):$.get("round/resend",{roundId:r._id})}),$(".speech").on("input",function(){var e=$(this).find("#index").length?$(this).find("#index"):$("<div>").attr("id","index").css("margin-top","-20px").css("position","absolute").prependTo($(".speech.active")),t=$(this).find("h1")
e.empty()
...
n.empty(),n.append(i),e.originalEvent.dataTransfer.effectAllowed="copy",$("#round .tab-pane").bind("dragover",!1)}),$("#docs").bind("dragend",function(e){$("#drag-copy").empty()}),$(".speech-nav").bind("dragover",!1).bind("drop",function(e){"false"!=$("#"+e.target.href.replace(/.+\#/gi,"")).attr("contenteditable")&&0!=$("#drag-copy").html().length&&(e.preventDefault(),e.stopPropagation(),$("#"+e.target.href.replace(/.+\#/gi,"")).append($("#drag-copy").html()),$("#drag-copy").empty())}),$(".speech").bind("drop",function(e){return"false"!=$(this).attr("contenteditable")&&0!=$("#drag-copy").html().length?(e.preventDefault(),e.stopPropagation(),$(e.target).append($("#drag-copy").html()),$("#drag-copy").empty(),$("#round .tab-pane").unbind("dragover"),!1):void 0}),$("#speech-flow-mode").click(function(){$(".active-speech").toggleClass("flow-mode"),$(".speech").attr("contenteditable","true"),$(".flow-mode").attr("contenteditable","false"),$(".active-nav i").remove(),$(".active-nav").hasClass("scroll")?$(".active-nav").removeClass("scroll").prepend('<i class="fa fa-fw fa-pencil"></i>'):$(".active-nav").addClass("scroll").prepend('<i class="fa fa-fw fa-cloud-download"></i>'),$(".active-speech").scrollTop($(".active-speech").scrollTop()+Math.floor(.99*$(".active-speech").height())),$(".active-speech").trigger("scroll")}),$("#fullscreen").click(function(){$("#round").css("max-width","100%"),$("#sidebar, #docs").hide()}),$("#speechscroll").click(function(){$(".active-nav i").remove(),$(".active-nav").hasClass("scroll")?$(".active-nav").removeClass("scroll").prepend('<i class="fa fa-fw fa-eye-slash"></i>'):$(".active-nav").addClass("scroll").prepend('<i class="fa fa-fw fa-eye"></i>'),$(".active-speech").scrollTop($(".active-speech").scrollTop()+Math.floor(.99*$(".active-speech").height())),$(".active-speech").trigger("scroll")})}),$(document).ready(function(){$("#searchtext").select2({ajax:{url:"/doc/search",dataType:"json",delay:50,data:function(e){return $("#searchtext").data("term",e.term),{q:e.term}},processResults:function(e,t){var n=t.term.toLowerCase().split(" "),i=u.index,o=u.index.map(function(e){return e.children||[]})
...
setTimeout(n,1500)})}),NodeList.prototype.forEach=Array.prototype.forEach
var ft={init:function(e,t){function n(){var e=document.elementFromPoint(filetree.offsetWidth+20,filetree.getBoundingClientRect().top+.2*filetree.clientHeight+30),t=$(e).closest("section>*").eq(0).prevUntil("h1,h2,h3").last(),n=$("#doc-"+ft.selected.id).find("h1, h2, h3")
if(this.worked=0,t){for(var i=0;i<n.length;i++)if(n.eq(i).text().length>0&&n.eq(i).is(t)){this.worked=1
break}this.worked&&($(".selected").removeClass("selected"),$("#"+ft.selected.id).find("li").eq(i-2).addClass("selected"),$(".selected")[0]&&document.body.scrollIntoViewIfNeeded&&$(".selected")[0].scrollIntoViewIfNeeded())}}ft.root=$(e),ft.dragging=!1,ft.selected={},ft.populate(e,t),$("#filetree").on("click","ul,li",ft.click),$("#filetree").on("click","label",function(e){var t=$(this).parent()
t.toggleClass("collapsed")}),$("#filetree").scroll(function(){this.scrollLeft=0}),$(window).on("scroll",function(){clearTimeout(window.scrollThrottle),window.scrollThrottle=setTimeout(n,500)}),setInterval(function(){ft.update()},2e4),ft.updateNeeded=!1,$("#docs").keyup(function(e){ft.updateNeeded=!0,$(window.getSelection().anchorNode).closest("h1,h2,h3").length&&(ft.updateNeeded=!0)}),$("#filetree").on("dragstart",function(e){ft.dragging=$(e.target).closest("nav"),$(e.target).addClass("ft-being-dragged")}),$(document).on("dragover",function(e){e.preventDefault()}),$("#filetree").on("dragenter",function(e){e=$(e.target).closest("nav"),e&&e.addClass("ft-dragged-over")}).on("dragleave",function(e){e=$(e.target).closest("nav"),e&&e.removeClass("ft-dragged-over")}).on("drop",function(e){if(!e.originalEvent.dataTransfer||!e.originalEvent.dataTransferuls.length){e.preventDefault()
...
return e=i.concat(e),{results:e}},cache:!0},minimumInputLength:1,escapeMarkup:function(e){return e},templateResult:function(e){return e.matchedString?"<span style='font-size: 80%'><b>"+e.text+"</b> <br><span style='font-size: 70%'>"+e.matchedString.replace(/<[^>]*>/gi,"")+"</span></span>":"<span><b>"+e.text+"</b> </span>"}}).on("select2:select",function(e){console.log(e.params.data),ft.click(e.params.data.id)
var t=$("#searchtext").data("term"),n=function(){t&&window.find(t,0,0,0,0,0,1),getSelection().anchorNode&&getSelection().anchorNode.parentNode.scrollIntoView()}
setTimeout(n,1500)})}),$(document).ready(function(){function e(){window.timerInterval=setInterval(function(){if(!(0>=o)){o--,o||($("#btn-play-container").click(),beep_final.play(),$("body").append('<div id="timeup" style="position: absolute; height: 100%; width: 100%;top: 0;left: 0;z-index: 9999;background-color: red;"></div>'),setTimeout(function(){$("#timeup").remove()},2e3)),$("#count").val(t(o))
...
if(c){var d=n.find("*[w\\:styleId='"+c.getAttribute("w:val")+"']")[0]
d&&inNodeChild.appendChild(d.cloneNode(!0))}inNodeChild.getElementsByTagName("b").length&&(l="<b>"+l+"</b>"),inNodeChild.getElementsByTagName("u").length&&(l="<u>"+l+"</u>"),inNodeChild.getElementsByTagName("highlight").length&&(l='<span style="background-color:'+inNodeChild.getElementsByTagName("highlight")[0].getAttribute("w:val")+'">'+l+"</span>"),s.innerHTML+=l}var c=inNode.getElementsByTagName("pPr").length?inNode.getElementsByTagName("pPr")[0].getElementsByTagName("pStyle")[0]:!1
if(c){var d=n.find("*[w\\:styleId='"+c.getAttribute("w:val")+"']")[0]
if(d&&(d.getElementsByTagName("b").length&&(s.innerHTML="<b>"+s.innerHTML+"</b>"),d.getElementsByTagName("u").length&&(s.innerHTML="<u>"+s.innerHTML+"</u>"),d.getElementsByTagName("outlineLvl").length)){var r=d.getElementsByTagName("outlineLvl")[0].getAttribute("w:val")
s.innerHTML="<h"+(parseInt(r)+1)+">"+s.innerHTML+"</h"+(parseInt(r)+1)+">"}}a.appendChild(s)}setTimeout(function(){$(".doc:visible").html(a.innerHTML)},300)}var n,o,a=e.target.result,s=new JSZip
