...
$(document).ready(function() {
  ...
  $("#showround").click();
    ...
    $("#speech-tabs > li:nth-child(3) > a").click()
  ...
  $("#sidebyside").click(function() {
    ...
    if ($(".speech.active").css("float") != "left") {
      ...
      ( $(".speech.active").attr('id')!="speech1NR" ? $(".speech.active").prev(".speech") : $(".speech.active").prev(".speech").prev(".speech") )
  ...
  $("body").on("click", ".flow-btn", function(){
    var _this = $(this);
    if (_this.hasClass("flow"))
      _this = _this.next();
      console.log(_this.attr('class'))

    if (_this.hasClass("flow-plus")){
      _this.removeClass("flow-plus")
...
// allow only "flowing input" for enemy's speeches
  ...
  /*  if (r.aff1 == u.email || r.aff2 == u.email)
    ...
    var el = $('<textarea type="text" class="flow" ></textarea><div class="flow-btn">[+]</div>');



    var p = $(window.getSelection().anchorNode).closest('p, .speech > *');

    if (p.prev().prev().hasClass("flow"))
      el = p.prev().prev();

    else if (p.prev().hasClass("flow"))
      el = p.prev();
    else
      p.before(el)
