...
var t=[]
return $(e).children("ul,li").each(function(){var e=$(this),n={id:e.attr("id"),title:e.children("label").text(),type:e.attr("class")},i=e.find("li")
i.length&&(n.children=ft.toJSON(e)),t.push(n)}),t},update:function(){if(u.index=ft.toJSON(),ft.selected&&$("#doc-"+ft.selected.id).length&&ft.updateNeeded&&ft.selected.id==$("#doc-"+ft.selected.id).attr("id").substring(4)){var e=[],t=0,n=-1
...
break}$("#"+ft.selected.id).find("li").remove(),ft.populate($("#"+ft.selected.id),e),$.post({url:"/doc/update",contentType:"application/json",data:JSON.stringify({text:$("#doc-"+ft.selected.id).html(),id:ft.selected.id})})}ft.updateIndex(),localStorage.index=filetree.innerHTML,document.cookie="index_updated="+Date.now()+"; expires=Fri, 18 Nov 2020 20:20:20 UTC; path=/"},updateIndex:function(){u.index.length&&u.name&&$.post({url:"/user/update",data:JSON.stringify({userid:u._id,index:u.index}),contentType:"application/json"})},click:function(e){function t(){$("#doc-"+ft.selected.id).find("h1, h2, h3")[a].scrollIntoView(),$("#doc-"+ft.selected.id).scrollTop($("#doc-"+ft.selected.id).scrollTop()-260)}e.stopPropagation()
...
"local"==u.index&&(filetree.innerHTML=localStorage.index,u.index=ft.toJSON()),ft.init($("#filetree"),u.index),setTimeout(function(){$.get("/html/interface.panels.html",function(e){$("body").append(e)})},2e3),$("#ft-minimize-unread").click(),$(window).on("popstate",loadFile)
var o=location.pathname.substr(1)
o&&"#"!=o?e(o):u.name&&"manual"!=u.index[0].id?$(".selected,.opened").length&&$(".selected,.opened").eq(0).click():($(".ft-item:first").click(),$("#docs").load("/html/manual.html"),$.get("/doc/read",{id:"manual"},function(e){$("#docs").html(e)}),window.scrollTo(0,0)),$("body").on("touchstart",function(e){window.touchStart=e.originalEvent.changedTouches[0].pageX}).on("touchend",function(e){var t=window.touchStart?e.originalEvent.changedTouches[0].pageX-window.touchStart:0;-100>t?($("#sidebar").animate({marginLeft:"-"+$("#sidebar").width()},0),$("#docs").animate({paddingLeft:"0"},0)):t>100&&($("#sidebar").animate({marginLeft:"0"},0),$("#docs").animate({paddingLeft:"20%"},0))}),$(document).width()<700,window.dragSidebar=!1,$("#sidebar").on("mousemove",function(e){$(this).width()-e.offsetX<10?$("body").css("cursor","e-resize"):dragSidebar||$("body").css("cursor","")}).on("mouseout",function(e){dragSidebar||$("body").css("cursor","")}).on("mousedown touchstart",function(e){var t=e.originalEvent.touches?e.originalEvent.touches[0].clientX:e.offsetX
...
var t}),$("#format-highlight").click(function(){$("#docs").on("mouseenter",function(){$("#docs").off("mouseenter"),$("#superreadcard").hasClass("enabled")&&$("#sw")[0].play()}),$(this).toggleClass("active"),$("#docs, .speech").toggleClass("highlight-mode").trigger("mouseup")}),$("#format-remove").click(function(){var e=window.getSelection().getRangeAt(0),t=e.toString(),n=document.createElement("span")
...
$(".ft-being-dragged:first").closest(".ft-item").remove(),u.index=ft.toJSON(),ft.updateIndex()}})}),$(document).ready(function(){$("#round").on("mousedown click",".flow-mode",function(e){console.log(e)
