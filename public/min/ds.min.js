...
t.toggleClass("collapsed")}),$("#filetree").scroll(function(){}),$("#filetree").mousemove(function(e){}),ft.updateNeeded=!1,$("#docs").keyup(function(e){ft.updateNeeded=!0,$(window.getSelection().anchorNode).closest("h1,h2,h3").length&&(ft.updateNeeded=!0)})},populate:function(e,t){function n(e){var t=""
...
break}$("#"+ft.selected.id).find("li").remove(),ft.populate($("#"+ft.selected.id),e),$.post({url:"/doc/update",contentType:"application/json",data:JSON.stringify({text:$("#doc-"+ft.selected.id).html(),id:ft.selected.id})})}ft.updateIndex()},updateIndex:function(){u.index.length&&u.name&&$.post({url:"/user/update",data:JSON.stringify({userid:u._id,index:u.index}),contentType:"application/json"})},click:function(e){function t(){for(var e=$("#doc-"+ft.selected.id).find("h1, h2, h3"),t=0;t<e.length;t++)if(e[t].textContent.length<3)e.splice(t--,1)
else if(t==a)break
...
setTimeout(function(){$.get("/html/interface.panels.html",function(e){$("body").append(e),initRoundPanel(),initTimer(),initFlow(),location.pathname.length<2&&$("#showround").click()})},o),$("#ft-minimize-unread").click(),$(window).on("popstate",loadFile)
var a=location.pathname.substr(1)
a&&"#"!=a?e(a):u.name&&"manual"!=u.index[0].id?$(".selected,.opened").length&&loadFile($(".selected,.opened").eq(0).attr("id")):($(".ft-item:first").click(),$("#docs").load("/html/manual.html"),window.scrollTo(0,0))}),$(document).ready(function(){$("body").on("touchstart",function(e){window.touchStart=e.originalEvent.changedTouches[0].pageX}).on("touchend",function(e){var t=window.touchStart?e.originalEvent.changedTouches[0].pageX-window.touchStart:0;-100>t?($("#sidebar").animate({marginLeft:"-"+$("#sidebar").width()},0),$("#docs").animate({paddingLeft:"0"},0)):t>100&&($("#sidebar").animate({marginLeft:"0"},0),$("#docs").animate({paddingLeft:"20%"},0))}),window.innerWidth<=800&&$('<meta name="theme-color" content="#AACFE7"/>  <meta name="viewport" content="minimal-ui, width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0"/>  <meta name="mobile-web-app-capable" content="yes"/>').prependTo("head"),$(document).width()<700,window.dragSidebar=!1,$("#sidebar").on("mousemove",function(e){$(this).width()-e.offsetX<10?$("body").css("cursor","e-resize"):dragSidebar||$("body").css("cursor","")}).on("mouseout",function(e){dragSidebar||$("body").css("cursor","")}).on("mousedown touchstart",function(e){var t=e.originalEvent.touches?e.originalEvent.touches[0].clientX:e.offsetX
